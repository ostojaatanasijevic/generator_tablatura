...
pub fn threaded_note_convolution(
    song: &Vec<i16>,
    note: &Note,
    window_type: &str,
...
pub fn note_convolution(
    ...
    for c in 0..num_of_chunks {
        ...
        let decemeted = block_max_decemation(
            &output[c * sample_len..(c + 1) * sample_len].to_vec(),
            sample_len / 4,
        );
...
pub fn dtft<T: unsinkable>(input_chunk: &Vec<T>, window: &Vec<f32>, nfft: usize) -> Vec<f32> {
    let mut planner = FftPlanner::<f32>::new();
...
pub fn cross_corr_notes(
    ...
    s_buffer = first_fft_data
        .iter()
        .zip(second_fft_data.iter())
        .map(|(x, y)| x * y.conj())
        .collect();
