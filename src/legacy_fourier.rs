...
pub fn convolution_per_note(input_chunk: &Vec<i16>,
                     sample_ffts: &Vec<Vec<Vec<Complex<f32>>>>,
                     window: &Vec<f32>,
                     convolution_type: &str
                     ) -> Vec<Vec<Vec<f32>>>{ 
    ...
    for string in 0..sample_ffts.len(){
        for note in 0..sample_ffts[0].len(){
            ...
            for c in 0..num_of_chunks{
                ...
                for i in 0..SAMPLE{ pesma_fft[i].re = (input_chunk[c*SAMPLE+ i] as f32) * window[i] / 65536.0; }

                fft.process(&mut pesma_fft);
    
                let mut s_buffer: Vec<Complex<f32>> = pesma_fft.iter().zip(sample_ffts[string][note].iter())
                ...
                for i in 0..current.len(){ out[c * SAMPLE + i] += current[i]; }
